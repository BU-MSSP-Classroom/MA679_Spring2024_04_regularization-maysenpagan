...
#ames_raw2$`Overall Qual` <- scale(ames_raw2$`Overall Qual`)
ames_raw2$`Lot Area` <- scale(log(ames_raw2$`Lot Area`))
ames_raw2$SalePrice <- scale(log(ames_raw2$SalePrice))
#change year remod/add to years since minimum year (1950)
min_year <- min(unique(ames_raw2$`Year Remod/Add`))
ames_raw2$`Year Remod/Add` <- ames_raw2$`Year Remod/Add`-min_year
#standardized years since 1950
ames_raw2$`Year Remod/Add` <- scale(ames_raw2$`Year Remod/Add`)
#one hot encode foundation variable
# dummy <- dummyVars(" ~ Foundation", data=ames_raw2)
# final_df <- data.frame(predict(dummy, newdata=ames_raw2))
# ames_raw2 <- cbind(ames_raw2, final_df)

ames_raw_2008=ames_raw2[ames_raw2$`Yr Sold`<2008,]
ames_raw_2009=ames_raw2[ames_raw2$`Yr Sold`>=2008,]
```
...
f <- as.formula(SalePrice ~ `Overall Qual` + Foundation + `Year Remod/Add` + (`TotRms AbvGrd`*`Lot Area`) + (`Garage Cars`*`Lot Area`))

y = new_2008$SalePrice
x = model.matrix(f, new_2008)[,-1]
set.seed(123)
...
newx <- model.matrix(f, ames_raw_2009)[,-1]
...
~~~
Based on the calculated loss for each method it appears that LASSO performs better in prediction than RIDGE regression.
~~~
...
reg_fit <- regsubsets(x = formula, data = sim_data)
